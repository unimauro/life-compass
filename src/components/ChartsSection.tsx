    ...
    <div className="bg-card border border-border rounded-xl p-3 shadow-soft text-sm">
      ...
      {payload.map((p: any) => (
        <p key={p.name} style={{ color: p.color || p.fill }} className="mt-1">
          {p.name}: {p.value}{typeof p.value === 'number' && p.value < 100 ? (label ? ' per 100k' : '%') : ''}
        </p>
      ))}
    </div>
  );
    ...
    <section className="px-6 py-24 bg-muted/30">
      <div className="max-w-6xl mx-auto">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">Data & Trends</h2>
          <p className="text-muted-foreground mt-3 text-lg max-w-xl mx-auto">
            The global trend is declining — a sign that prevention works.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h3 className="text-lg font-semibold mb-6">Global Trend (2000–2020)</h3>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={globalTrend}>
                ...
                <Line type="monotone" dataKey="rate" name="Rate per 100k" stroke="hsl(173, 80%, 30%)" strokeWidth={3} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </motion.div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h3 className="text-lg font-semibold mb-6">Distribution by Age Group</h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h3 className="text-lg font-semibold mb-6">Top 15 Countries by Suicide Rate</h3>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={topCountries} layout="vertical">
                ...
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="rate" name="Rate per 100k" fill="hsl(173, 80%, 35%)" radius={[0, 6, 6, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </motion.div>
        </div>
